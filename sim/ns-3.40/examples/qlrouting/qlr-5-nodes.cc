...
void
startTcpRtx(Ptr<Node> node, std::string fileName, std::string socketId)
{
    std::string nsString = "/NodeList/" + std::to_string(node->GetId()) +
                           "/$ns3::TcpL4Protocol/SocketList/" + socketId + "/Rx";

    std::cout << "nsString: " << nsString << std::endl;
             ...
             std::string congestionControl = "TcpLinuxReno")
{
    ...
    if (rtxPath != "")
    {
        ...
        NS_LOG_INFO("Starting TCP Retransmission tracking for " + Names::FindName(senderHost) +
                    " -> " + Names::FindName(receiverHost));
        std::string fileName = Names::FindName(senderHost) + "-" + Names::FindName(receiverHost) +
                               "-" + std::to_string(port);
        NS_LOG_INFO("Saving results in: " + rtxPath);

        Simulator::Schedule(Seconds(1.1),
                            &startTcpRtx,
                            receiverHost,
                            getPath(rtxPath, fileName + ".rtx"),
                            "0"
                        );

        Simulator::Schedule(Seconds(0),
                            &startThroughputTrace,
                            getPath(rtxPath, fileName + ".tp"),
                            receiverHost->GetId(),
                            0);

        Simulator::Schedule(Seconds(1.1),
                            &TraceCwnd,
                            getPath(rtxPath, fileName + ".cwnd"),
                            senderHost->GetId(),
                            0);

        firstCwnd[senderHost->GetId()] = true;
        cWndStream[senderHost->GetId()] =
            Create<OutputStreamWrapper>(getPath(rtxPath, fileName + ".cwnd").c_str(),
                                        std::ios::out);
    }
    else
    {
        NS_LOG_INFO("No TCP Retransmission tracking for " + Names::FindName(senderHost) + " -> " +
                    Names::FindName(receiverHost));
    }
}
...

{
    ...
    std::string rtxPath = resultsPath;
