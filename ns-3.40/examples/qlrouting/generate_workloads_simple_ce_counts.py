...
def _congestion_events_per_destination(
    ...
    ``events_per_dest`` floods, each on a link of a different path toward the
    destination (successive-path blocking: block a link, recompute the path
    on the residual DAG, block a link of that new path, ...). A route to the
    destination is always left open (never disconnect) — that is the path
    the global scheme can still find, while a purely-local scheme reroutes
    from one blocked path into another that is congested at the same time.

    Events for the same destination are simultaneous but not identical: each
    gets its own randomized start time/duration, constrained to all contain
    one shared instant (``overlap_point``) so they are guaranteed to overlap
    -- staggered onset/offset instead of byte-for-byte identical windows.
    """
    import sys
    ...
    for k, dst in enumerate(destinations):
        ...
        # Shared instant every event for this destination must be active at
        # -- guarantees overlap (true simultaneity) even though each event
        # below draws its own start/duration independently.
        overlap_point = slot_start + slot_len * rng.uniform(0.4, 0.6)
        ...
        for _ in range(events_per_dest):
            ...
            # Per-event start/duration, constrained to contain overlap_point
            # so this event overlaps every other event for this destination
            # without sharing an identical window.
            duration = min(rng.uniform(duration_min, duration_max), slot_len)
            earliest_start = max(slot_start, overlap_point - duration)
            latest_start = min(overlap_point, slot_end - duration)
            if latest_start < earliest_start:
                start_time = max(slot_start, min(overlap_point, slot_end - duration))
            else:
                start_time = rng.uniform(earliest_start, latest_start)
